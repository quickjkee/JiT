...
class SSLMetaArch(nn.Module):
    ...
    def forward_backward(self, images, teacher_temp):
        ...
        global_class_idxs = images['global_targets'].cuda(non_blocking=True)
        local_class_idxs = images['local_targets'].cuda(non_blocking=True)
        
        batch_size = len(global_crops) // n_global_crops
        global_t = torch.randn(batch_size, 1, 1, 1, device=global_crops.device, dtype=global_crops.dtype).repeat(n_global_crops, 1, 1, 1)
        global_t = torch.sigmoid(global_t * 0.8 - 0.8)
        
        local_t = torch.randn(batch_size, 1, 1, 1, device=local_crops.device, dtype=local_crops.dtype).repeat(n_local_crops, 1, 1, 1)
        local_t = torch.sigmoid(local_t * 0.8 - 0.8)
        ...
        def get_teacher_output():
            ...
            teacher_backbone_output_dict = self.teacher.backbone(x, is_training=True, class_idxs=global_class_idxs, t=global_t)
        ...
        student_global_backbone_output_dict, student_local_backbone_output_dict = self.student.backbone(
            [global_crops, local_crops], masks=[masks, None], class_idxs=[global_class_idxs, local_class_idxs], 
            t=[global_t, local_t], is_training=True,
        )
