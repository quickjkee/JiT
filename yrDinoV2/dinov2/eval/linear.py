...
def get_args_parser(
    ...
    # Auxilary args to init train config. NOT USED FOR LINEAR EVAL
    parser.add_argument(
        "--data-path",
        type=str,
        default="",
        help="Not used for linear tuning"
    )
    parser.add_argument(
        "--batch-size-per-gpu",
        type=int,
...
def make_eval_data_loader(test_dataset_str, batch_size, num_workers, metric_type):
    ...
    test_dataset.samples = [
        x for i in range(0, len(test_dataset), 50) for x in test_dataset.samples[i:i+10]
    ]
...
def run_eval_linear(
    ...
    training_num_classes = len(torch.unique(torch.tensor([y for _, y in train_dataset.samples]).to(int)))
