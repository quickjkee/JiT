...
from torch.distributed import barrier, broadcast, is_initialized, all_reduce, all_gather, ReduceOp, broadcast_object_list
...
VERBOSE = True
def rank_aware_print(*args, sep=' ', end='\n', file=None, flush=False):
    """Rank-aware print

    Args:
        sep (str, optional):
        end (str, optional):
        file (PathLike or None, optional):
        flush (bool, optional):
    """
    ...
    if not dist.is_initialized():
        ...
        rank_aware_prefix = f"[LOCAL RANK: {local_rank + 1}/{GPUS_PER_NODE}\tGLOBAL RANK: {global_rank + 1}/{world_size}]\t\t"
        print(rank_aware_prefix, *args, sep=sep, end=end, file=file, flush=True)